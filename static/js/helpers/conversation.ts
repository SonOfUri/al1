...
import type { TETHAddress } from "./string";
import { throttledFetchEnsName, throttledFetchAddressName } from "./string";
  ...
  if (!name) {
    name =
      (await throttledFetchAddressName(
        conversation.peerAddress as TETHAddress
      )) ?? null;
  }
  return name;
};
...
export const fetchPeerAddressAvatar = async (
  ...
  if (!avatar) {
    ...
    if (name) {
      ...
      avatar = `https://effigy.im/a/${name}.svg`;
    }
  }
  return avatar;
};
...
export const updateConversationIdentities = async (
  ...
  if (addressesWithoutNames.length > 0) {
    ...
    for (const chunk of chunkArray(addressesWithoutNames, 10)) {
